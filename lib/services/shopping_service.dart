...
class ShoppingService {
  ...
  // ============== SMART RECURRING LIST OPERATIONS ==============

  /// Get all smart recurring lists for the family
  Future<List<SmartRecurringList>> getSmartRecurringLists({bool forceRefresh = false}) async {
    final familyId = await _familyId;
    if (familyId == null) {
    ...
    try {
      ...
      return snapshot.docs.map((doc) {
        try {
          return SmartRecurringList.fromJson({
            'id': doc.id,
            ...doc.data(),
          });
        } catch (e, st) {
          Logger.warning('getSmartRecurringLists: Error parsing list ${doc.id}', error: e, stackTrace: st, tag: 'ShoppingService');
          return null;
        }
      }).whereType<SmartRecurringList>().toList();
    } catch (e, st) {
  ...
  /// Stream smart recurring lists for real-time updates
  Stream<List<SmartRecurringList>> streamSmartRecurringLists() {
    return Stream.fromFuture(_familyId).asyncExpand((familyId) {
      if (familyId == null) {
        return Stream.value(<SmartRecurringList>[]);
      }
      ...
      return _firestore
          ...
          .snapshots()
          .map((snapshot) => snapshot.docs
              .map((doc) => SmartRecurringList.fromJson({
                    'id': doc.id,
                    ...doc.data(),
                  }))
              .toList());
  ...
  /// Create or update a smart recurring list based on completed shopping list
  Future<SmartRecurringList> createOrUpdateSmartList({
    required String listId,
    String? name,
    ...
    // Get items from the completed list
    final items = await getShoppingItems(listId);
    final purchasedItems = items.where((item) => item.isPurchased).toList();

    if (purchasedItems.isEmpty) {
      throw ValidationException('Cannot create smart list from empty or unpurchased items', code: 'no-items');
    }

    // Generate list name if not provided
    final smartListName = name ?? 'Weekly Groceries';

    // Check if a smart list with similar items already exists
    final existingLists = await getSmartRecurringLists();
    SmartRecurringList? existingList;

    for (var list in existingLists) {
      if (list.name == smartListName && list.frequency == frequency) {
        existingList = list;
        break;
      }
    }

    final itemNames = purchasedItems.map((item) => item.name).toList();
    final itemFrequencies = <String, int>{};

    // Count item frequencies
    for (var item in purchasedItems) {
      itemFrequencies[item.name] = (itemFrequencies[item.name] ?? 0) + 1;
    }

    if (existingList != null) {
      // Update existing smart list
      final updatedItemNames = <String>[...existingList.itemNames];
      final updatedFrequencies = Map<String, int>.from(existingList.itemFrequencies);

      // Merge new items
      for (var itemName in itemNames) {
        if (!updatedItemNames.contains(itemName)) {
          updatedItemNames.add(itemName);
        }
        updatedFrequencies[itemName] = (updatedFrequencies[itemName] ?? 0) + 1;
      }

      final updatedList = existingList.copyWith(
        itemNames: updatedItemNames,
        itemFrequencies: updatedFrequencies,
        usageCount: existingList.usageCount + 1,
      ...
      Logger.info('createOrUpdateSmartList: Updated smart list ${existingList.id}', tag: 'ShoppingService');
      return updatedList;
    } else {
      // Create new smart list
      final smartListId = _uuid.v4();
      final now = DateTime.now();

      final smartList = SmartRecurringList(
        id: smartListId,
        familyId: familyId,
        name: smartListName,
        itemNames: itemNames,
      ...
      Logger.info('createOrUpdateSmartList: Created smart list $smartListId', tag: 'ShoppingService');
      return smartList;
    }
  }

  /// Use a smart recurring list to create a new shopping list
  Future<ShoppingList> createListFromSmartRecurringList(String smartListId) async {
    final familyId = await _familyId;
    if (familyId == null) throw AuthException('User not part of a family', code: 'no-family');
    ...
    // Create new shopping list from smart list
    final newList = await createShoppingList(
      name: smartList.name,
      description: 'Created from smart list',
    );
    ...
    Logger.info('createListFromSmartRecurringList: Created list from smart list $smartListId', tag: 'ShoppingService');
    return newList;
  }

  /// Delete a smart recurring list
  Future<void> deleteSmartRecurringList(String smartListId) async {
    final familyId = await _familyId;
    if (familyId == null) throw AuthException('User not part of a family', code: 'no-family');
    ...
    Logger.info('deleteSmartRecurringList: Deleted smart list $smartListId', tag: 'ShoppingService');
  }
