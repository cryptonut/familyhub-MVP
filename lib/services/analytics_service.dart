  ...
  Future<FamilyAnalytics> getFamilyAnalytics({
    ...
    try {
      ...
      return FamilyAnalytics(
        ...
        periodEnd: endDate,
        // Task analytics
        totalTasks: (taskData['totalTasks'] as num?)?.toInt() ?? 0,
        completedTasks: (taskData['completedTasks'] as num?)?.toInt() ?? 0,
        activeTasks: (taskData['activeTasks'] as num?)?.toInt() ?? 0,
        taskCompletionRate: (taskData['completionRate'] as num?)?.toDouble() ?? 0.0,
        tasksByMember: (taskData['tasksByMember'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toInt())) ?? <String, int>{},
        averageCompletionTime: (taskData['averageCompletionTime'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toDouble())) ?? <String, double>{},
        // Message analytics
        totalMessages: (messageData['totalMessages'] as num?)?.toInt() ?? 0,
        messagesByMember: (messageData['messagesByMember'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toInt())) ?? <String, int>{},
        messageActivityByHour: (messageData['activityByHour'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toInt())) ?? <String, int>{},
        // Calendar analytics
        totalEvents: (calendarData['totalEvents'] as num?)?.toInt() ?? 0,
        upcomingEvents: (calendarData['upcomingEvents'] as num?)?.toInt() ?? 0,
        eventsByType: (calendarData['eventsByType'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toInt())) ?? <String, int>{},
        eventAttendanceRate: (calendarData['attendanceRate'] as num?)?.toDouble() ?? 0.0,
        // Photo analytics
        totalPhotos: (photoData['totalPhotos'] as num?)?.toInt() ?? 0,
        photosByMember: (photoData['photosByMember'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toInt())) ?? <String, int>{},
        photoUploadsByDay: (photoData['uploadsByDay'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toInt())) ?? <String, int>{},
        // Game analytics
        totalGamesPlayed: (gameData['totalGames'] as num?)?.toInt() ?? 0,
        gamesWonByMember: (gameData['winsByMember'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toInt())) ?? <String, int>{},
        averageGameDuration: (gameData['averageDuration'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toDouble())) ?? <String, double>{},
        // Wallet analytics
        totalEarned: (walletData['totalEarned'] as num?)?.toDouble() ?? 0.0,
        totalSpent: (walletData['totalSpent'] as num?)?.toDouble() ?? 0.0,
        earningsByMember: (walletData['earningsByMember'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toDouble())) ?? <String, double>{},
        recentTransactions: (walletData['recentTransactions'] as List<dynamic>?)?.map((e) => e as WalletTransaction).toList() ?? <WalletTransaction>[],
        // Activity trends
        dailyActivity: (activityData['dailyActivity'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toInt())) ?? <String, int>{},
        weeklyActivity: (activityData['weeklyActivity'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, (v as num).toInt())) ?? <String, int>{},
      );
    } catch (e, st) {
