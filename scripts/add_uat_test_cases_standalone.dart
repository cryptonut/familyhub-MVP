...
/// 
/// This version uses HTTP requests directly, avoiding Flutter dependencies
/// 
/// Usage: dart scripts/add_uat_test_cases_standalone.dart [environment] [access_token]
/// 
...
const String firestoreBaseUrl = 'https://firestore.googleapis.com/v1/projects/$firebaseProjectId/databases/(default)/documents';

// Test cases data structure (same as original)
final testCasesData = [
  {
    'number': 1,
...
/// Convert a Map to Firestore document format
...
Future<String> createDocument(
  ...
  request.write(jsonEncode(firestoreData));
...
Future<void> main(List<String> args) async {
  ...
  if (accessToken == null || accessToken.isEmpty) {
    ...
    print('  3. Get token: firebase login:ci');
    print('  4. Run: dart scripts/add_uat_test_cases_standalone.dart dev YOUR_TOKEN');
    print('');
    print('OR set environment variable:');
    print('  set FIREBASE_ACCESS_TOKEN=YOUR_TOKEN');
    print('  dart scripts/add_uat_test_cases_standalone.dart dev');
    print('');
    print('OR use the Flutter version (recommended for now):');
    print('  flutter run scripts/add_uat_test_cases.dart dev');
    print('');
    exit(1);
  }
  ...
  try {
    ...
    if (e.toString().contains('401') || e.toString().contains('403')) {
      ...
      print('Get a new token: firebase login:ci');
    }
    print('');
    exit(1);
  }
