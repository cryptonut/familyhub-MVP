# Enhanced PowerShell script to push a commit, update firewall rules, and capture ALL possible logs on working PC
# WARNING: This may log sensitive information like tokens or passwords. Review and secure the log file!

$LOG_FILE = "working_pc_full_logs_including_auth.txt"
...
Write-Host "Ping to api.github.com:"
Test-Connection -ComputerName api.github.com -Count 4
Write-Host "Curl test to github.com (verbose):"
...
if (Test-Path "$env:USERPROFILE\.git-credentials") {
    Write-Host "File exists - reviewing structure (not showing full content for security):"
    $credFile = Get-Content "$env:USERPROFILE\.git-credentials"
    Write-Host "Line count: $($credFile.Count)"
    Write-Host "First line pattern: $($credFile[0] -replace ':[^:]+@', ':****@')"
} else {
...
Get-EventLog -LogName System -After (Get-Date).AddHours(-1) -EntryType Error,Warning -ErrorAction SilentlyContinue | Select-Object -First 20 | Format-Table
Get-EventLog -LogName Application -After (Get-Date).AddHours(-1) -EntryType Error,Warning -ErrorAction SilentlyContinue | Select-Object -First 20 | Format-Table
